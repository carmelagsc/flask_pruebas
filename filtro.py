...
# --- Guardas de quietud (nuevas) ---
NOISE_STD_MIN = 0.30       # [m/s^2]
PP_MIN        = 1.00       # [m/s^2]
PROM_ABS_MIN  = 0.50       # [m/s^2]
MIN_PEAKS_GATE = 3         # [picos] en 10 s
...
def compute_counts_depth(arr):
    ...
    sig_pp  = float(np.ptp(sig))
    if (sig_std < NOISE_STD_MIN) or (sig_pp < PP_MIN):
        return 0, 0.0, 0.0
